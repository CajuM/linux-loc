...
# From https://raw.githubusercontent.com/zegelin/qemu-affinity/refs/heads/master/qemu_affinity.py
...
AFFINITY_SPEC_RE = re.compile(r'(\d+)(?:-(\d+))?$')
...
def qemu_proc(pid):
	...
	exe = (p / 'exe').resolve()
	if not QEMU_COMM_RE.findall(exe.name):
...
def main():
	...
	classes = [
		...
		['--kvm-affinity', re.compile(r'^CPU (?P<name>\d+)/KVM$'), 'CPU %s/KVM'],
	...
	for (arg, thread_name_re, thread_name_fmt) in classes:
		...
		# [thread] -> [(thread, match)]
		threads = list(map(lambda t: (t, thread_name_re.match(t.name)), args.qemu_proc.threads.values()))
		threads = list(filter(lambda v: v[1] is not None, threads))
