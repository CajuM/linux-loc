...
for vmlinuz in ${vmlinuz_list}; do
	...
	while [ ${loop} -eq 1 ]; do
		...
		rx_pid=$!
		sleep 2
		./qemu_affinity.py -k 1,2 -- ${rx_pid}

		qemu_vm "${vmlinuz}" tx pc > "out/exp-tx-${kver}-1.log" &
		tx_pid=$!
		sleep 2
		./qemu_affinity.py -k 3,4 -- ${tx_pid}

		last3=0
		last2=0
		last1=0
		while :; do
			len=$(grep '^bps=' "out/exp-tx-${kver}-2.log" | wc -l)
	...
	while [ ${loop} -eq 1 ]; do
		...
		tx_pid=$!
		sleep 2
		./qemu_affinity.py -k 3,4 -- ${rx_pid}

		last3=0
		last2=0
		last1=0
		while :; do
			len=$(grep '^bps=' "out/exp-rx-${kver}-2.log" | wc -l)
			if [[ ( ${len} == ${last3} ) && ( ${len} != 0 ) ]]; then
				break
			fi
